...
class Chaplin:
    def __init__(self, voice_sample_path=None, tts_speaker="Claribel Dervla", camera_index=0, meeting_context="", persist_vector_db=True, gui_mode=False, status_callback=None):
        self.vsr_model = None
        self.camera_index = camera_index
        self.persist_vector_db = persist_vector_db
        self.gui_mode = gui_mode  # If True, don't type output (for GUI)
        self.status_callback = status_callback  # Callback for status updates to GUI
        self.kbd_controller = keyboard.Controller() if not gui_mode else None
    ...
    async def correct_output_async(self, output, sequence_num):
        ...
        print("\033[48;5;94m\033[97m\033[1m CORRECTING WITH LLM... \033[0m")
        if self.status_callback:
            self.status_callback("🤖 Correcting with LLM...")
...
Return the corrected text in the format of 'list_of_changes' and 'corrected_text'."""
        ...
        if self.status_callback:
            self.status_callback("✓ LLM correction complete")
        ...
        async with self.typing_condition:
            ...
            if not self.waiting_for_input:
                # In GUI mode, skip typing (output goes to transcript panel)
                if not self.gui_mode:
                    ...
                    print(f"\n\033[48;5;33m\033[97m\033[1m CORRECTED TEXT \033[0m: \"{chat_output.corrected_text.strip()}\"")
                    if self.status_callback:
                        self.status_callback(f"💬 Corrected: {chat_output.corrected_text.strip()}")
                
                # speak the corrected text using TTS (still blocking keys)
                print("\033[48;5;94m\033[97m\033[1m STARTING TTS... \033[0m")
                if self.status_callback:
                    self.status_callback("🎤 Starting TTS...")
    ...
    def _speak_text(self, text):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback(f"🎶 Generating speech: {text.strip()}")
            ...
            if self.status_callback:
                self.status_callback("🔊 Playing audio...")
            ...
            if self.status_callback:
                self.status_callback("✓ Audio playback complete")
